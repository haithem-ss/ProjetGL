...
const ProductPage = () => {
  ...
  const dataSumbit = async (e) => {
    ...
    await getAdresses(e).then(async (res) => {
      let id = 47;
      if (
        adresses.filter(
          (adresse) => adresse.id === id && adresse.wilaya === wilaya
        ).length === 0
      ) {
    ...
    const id = 4;
    const data = `id=${id}&niveau=${category}&modalité=${modalite}&description=${description}&titre=${moduleName}&tarif=${price}&tarifPromotion=${tarifPromotion}&lieuFormation=${lieu.id}&module=${module.id}&auteur=&thumbnail=`;
    const config = {
      headers: {
    ...
    await axios
      .put("http://localhost:8000/cours/", data, config)
      .then((res) => {
        console.log(res.data);
      })
      .catch((err) => {
