...
const ProductPage = () => {
  ...
  useEffect(() => {
    console.log("thumnail", thumbnail_url);
  }, [thumbnail_url]);
  ...
  const dataSumbit = async (e) => {
    ...
    const auteur_id = 1;
    const id = 1;
    const payload = `id=${id}&niveau=${category}&modalité=${modalite}&description=${description}&titre=your way to learn ${moduleName}&tarif=${price}&tarifPromotion=${tarifPromotion}&lieuFormation=${lieu.id}&module=${module.id}&auteur=${auteur_id}&thumnail_url=${thumbnail_url}`;
    ...
    axios
      .put("http://127.0.0.1:8000/cours/", payload, config)
