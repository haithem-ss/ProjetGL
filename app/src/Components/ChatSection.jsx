...
const DateMessages = ({ dte }) => {
  return (
    <Text margin="10px" color="#6C757D" fontSize="xs" textAlign="center">
      {dte}
    </Text>
  );
};
...
const TopBar = ({ currentChat }) => {
    ...
    >
      <Flex align="center" gap={3}>
        ...
        <Heading size="sm" fontWeight="700">
          {currentChat.user2.nom + " " + currentChat.user2.prenom}
        </Heading>
      </Flex>
      <Menu>
        <MenuButton
  ...
  const MessagesSection = ({ NewMessages, currentChat, senderID }) => {
    ...
    React.useEffect(() => {}, [NewMessages, Messages]);
      ...
      >
        ...
        <IconButton
          ...
          icon={
            ...
            >
              <path
                d="M0 32V19.75L15.1 16L0 12.15V0L38 16L0 32Z"
                fill="white"
              />
            </svg>
          }
    ...
    React.useEffect(() => {
      if (currentChat != null) {
        ...
        const socket = new WebSocket(
          "ws://localhost:8000/chat/" + currentChat.id + "/"
        );
        socket.onopen = (e) => {
          toast({
